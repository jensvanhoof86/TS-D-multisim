...
# =========================================================
# TCP broadcast server (for 10012 datastream / 10013 livetools)
# =========================================================
...
class TcpBroadcastServer:
    def __init__(self, port: int, name: str):
        ...
        self.thread.start()
        print(f"[{self.name}] Listening on 0.0.0.0:{self.port}")
...
parser.add_argument("--enable-livetools-server", action="store_true", help="Broadcast LiveTools NMEA on TCP 10013")
...
def send_livetools_tcp(hhmmss, lat, lon, alt, spd, crs, respect_serial_delays: bool):
    """
    If respect_serial_delays=True (when main mode is LIVETOOLS),
    we broadcast chunks in the same cadence as serial mode (0.05s sleeps).
    Otherwise we broadcast the same bytes as one combined packet (no extra delay).
    """
    if not livetools_server:
        return

    if not respect_serial_delays:
        livetools_server.send(build_livetools_burst_bytes(hhmmss, lat, lon, alt, spd, crs))
        return

    # cadence like serial LIVETOOLS4800
    livetools_server.send(build_gga(hhmmss, lat, lon, alt).encode())
    time.sleep(0.05)

    livetools_server.send(build_gsa().encode())
    time.sleep(0.05)

    for g in build_gsv():
        livetools_server.send(g.encode())
        time.sleep(0.05)

    livetools_server.send(build_rmc(hhmmss, lat, lon, spd, crs).encode())
...
while True:
    ...
    with open(args.file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # -------------------------------------------------
            # Optional LiveTools TCP server output (ID-filtered)
            # -------------------------------------------------
            if livetools_server and args.id and dsm_id == args.id:
                # If main mode is LIVETOOLS, we match serial cadence.
                # Otherwise, don't add extra sleeps; send one combined packet.
                send_livetools_tcp(hhmmss, lat, lon, alt, spd, crs, respect_serial_delays=(mode_name == "LIVETOOLS"))
